...
export class Money extends ValueObject<{ amount: number; currency: string }> {
  ...
  static create(amount: number, currency: string): Money {
    if (!Number.isFinite(amount) || amount < 0) {
      throw new CartDomainError('Amount must be non-negative', 'INVALID_MONEY_AMOUNT');
    }
    const normalizedCurrency = currency?.trim().toUpperCase();
    if (!normalizedCurrency || normalizedCurrency.length !== 3) {
      throw new CartDomainError('Currency must be a 3-letter ISO code', 'INVALID_MONEY_CURRENCY');
    }
