...
export class AssignRoleHandler {
  ...
  async execute(command: AssignRoleCommand): Promise<Result<void, IdentityApplicationError>> {
    const canAssign = await this.authorizationService.can(command.actorId, 'identity:role:assign');
    if (!canAssign) {
    ...
    const assignResult = identity.assignRole(role.id, role.getName());
    if (assignResult.isFailure) {
      return Result.fail(new IdentityApplicationError(assignResult.getError().message, assignResult.getError().code));
    }
