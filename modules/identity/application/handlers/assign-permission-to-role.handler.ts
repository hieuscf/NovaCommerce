...
export class AssignPermissionToRoleHandler {
  ...
  async execute(command: AssignPermissionToRoleCommand): Promise<Result<void, IdentityApplicationError>> {
    const canAssign = await this.authorizationService.can(command.actorId, 'identity:permission:assign');
    if (!canAssign) {
    ...
    await this.auditLogger.log({
      ...
      metadata: { permissionKey: key.value },
    });
