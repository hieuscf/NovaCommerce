...
| Admin (Next.js) | 🚧 | `apps/admin` — Alloy dashboard + Product/Order/Account/Seller Management + Seller Approval fixtures + `/accounts/roles` (Gateway Identity) + `/login` (`adminAuthClient` → Gateway `POST /auth/login`, `RequireAdminAuth`), dark sidebar + topbar, isolated session/API client, error boundaries, Vitest |
