...
| Frontend             | 🚧 In Progress |      88% | Architecture foundation + `@novacommerce/frontend` client; homepage + Alloy `/login` `/register` `/account` dashboard + `/shop` and `/shop/[category]` listing via Search API + Alloy `/products/[slug]` PDP + `/cart` + `/checkout` + `/orders` list/detail/confirmation; header search → `/shop?q=`; session restore + global 401/403; admin shell |
