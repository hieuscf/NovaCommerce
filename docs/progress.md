...
| Web Store (Next.js) | 🚧 | `apps/web` — homepage, Alloy `/login` `/register` `/account` dashboard (User Gateway profile/addresses/preferences + editable forms), `/shop` and `/shop/[category]` listing (filters/sort/pagination on mock catalog; empty/skeleton/error), `/products/[slug]` PDP (gallery/variants/reviews on mock catalog; loading/not-found/error), `/cart` (lines/qty/summary on mock cart; empty/skeleton/error), `/checkout` (customer/shipping/summary + Alloy Payment Gateway tiles/card/OTP mapped to Gateway `paymentProvider`; empty/skeleton/error; navigates to `/orders/confirmed`, no Payment API), `/orders` list (Gateway `GET /users/me/orders` via `orderClient`; detail/confirmation still on mock orders; loading/error/not-found), customer auth UI, default client → Gateway Identity, error boundaries, Vitest |
