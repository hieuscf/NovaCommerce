...
| Full account dashboard (profile, addresses, orders) | Profile, addresses, and notification preferences use Gateway `/users/me*` via `userClient` (create-on-404, edit/delete/default address, preference PATCH). Payment/loyalty/wishlist/recent-order rails remain empty until those APIs are wired. Sign out on `/account?section=security`. `/orders` list loads Gateway `GET /users/me/orders` via `orderClient`. Order detail/confirmation remain fixture-backed until those adapters are wired. |
