...
| `/seller` | `(store)` | Public Seller Center. Unregistered visitors complete the Alloy onboarding wizard (business, shop, verification, English terms, completion/waiting-for-approval). Verified sellers see the Alloy seller workspace: home dashboard (`?demo=registered`) or product management (`?demo=registered&section=products` with `tab`/`q`/`category`/`page` filters). Default status is presentation-only `unregistered`. Do not invent Seller APIs until the deferred Seller module has a Gateway adapter. |
