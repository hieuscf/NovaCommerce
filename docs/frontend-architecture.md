...
| `/seller` | `(store)` | Public Seller Center. Unregistered visitors must complete the Alloy onboarding wizard (business information, shop details, verification with selling-model cards and origin-document uploads, English terms & conditions document, complete). Registered sellers will see a different workspace later. Status is presentation-only (`unregistered`) until the deferred Seller module has a Gateway adapter — do not invent Seller APIs. |
