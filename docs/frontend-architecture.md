...
| `/accounts` | `(console)` | Alloy Account Management — KPI cards, search/status filters (`q`, `status`, `page` in URL), accounts table with Edit action that prompts lock/unlock via `PATCH /identities/:id` (`{ locked }`), pagination. Live Gateway `GET /identities` (Identity directory + summary counts). `/accounts?role=customer` and `/accounts?role=admin` redirect to `/accounts`. |
