...
- **Accounts (`/accounts`):** Alloy Account Management — breadcrumb, Create Account CTA, four KPI cards, filter bar (search/role/status/reset), dense accounts table (avatar, role/status/verified badges, dates, row actions), pagination. Sidebar Accounts expands to All Accounts / Customers / Admins / Roles & Permissions (`role` query filters)
