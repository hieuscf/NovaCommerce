...
- **Seller Center:** `/seller` — unregistered Alloy onboarding (promo aside + 5-step application: business, shop, verification with selling-model cards and origin-document uploads, English seller terms document, completion/waiting-for-approval summary). Registered verified workspace (`?demo=registered` preview): light Seller Center shell (topbar + sidebar), home dashboard (welcome, KPIs, revenue chart, recent orders, features, ads promo), and product management (`&section=products`: five KPIs, status tabs, filters, dense product table with score/SEO, SEO suggestion rail). No Seller API yet (`docs/domain-model.md` defers the module).
- **Merch shots:** compact category/product cards use `ProductShape` clay illustrations; shop listing still uses catalog photos.

### App: `apps/admin`

- **Layout:** `AdminShell` — dark navy sidebar (`slate-950`) + sticky topbar (search, notifications, theme toggle, language, admin profile, sign out), mobile sheet nav, console footer. Console routes wrapped by `RequireAdminAuth` (redirect → `/login`).
- **Login (`/login`):** Admin-branded auth shell (ops navy panel + form card; not customer Alloy split). Posts Gateway `POST /auth/login` via `adminAuthClient`; access token held in-memory `adminSession` until cookie sessions land.
- **Dashboard (`/`):** Alloy operations canvas — greeting + date range, four KPI cards with sparklines, sales overview dual-line chart (SVG), order-status donut, recent-orders table, top-selling products, sales-by-category bars, quick actions, grow-business promo banner
- **Accounts (`/accounts`):** Alloy Account Management — breadcrumb, Create Account CTA, four KPI cards, filter bar (search + labeled Status select + reset), dense accounts table (avatar, role/status/verified badges, dates, row action Edit → lock/unlock prompt), pagination. Live Gateway `GET /identities` and `PATCH /identities/:id` (`{ locked }`). Sidebar Accounts expands to All Accounts / Roles & Permissions. Legacy `/accounts?role=customer` and `/accounts?role=admin` redirect to `/accounts`.
- **Roles & Permissions (`/accounts/roles`):** Alloy RBAC **detail** — role card strip, selected role header + tabs (Permissions matrix / Users empty / Description), grant filters, right rail summary + quick actions. Wired to Gateway Identity roles/permissions; checkbox grant calls `POST /roles/:id/permissions`
- **Products (`/products`):** Alloy Product Management — breadcrumb, Import/Export/Add Product CTAs, four KPI cards (total/active/OOS/value), filter bar (search/category/brand/status/reset), dense products table (thumbnail, SKU, category, brand, price, stock, status badges, dates, eye view + row actions), pagination. Eye opens `/products/[id]` detail. Sidebar Products expands to All Products / Categories / Brands / Attributes / Reviews (sub-routes reserved). Fixtures only until Catalog admin Gateway
- **Product Detail (`/products/[id]`):** Alloy product review — gallery, product info, description tabs, media, related products; right rail for status change, lock toggle, violation lock CTA, save/view-on-store, report history. Edit mode toggles inline fields. Fixtures only
- **Orders (`/orders`):** Alloy Order Management — breadcrumb, Export/Create Order CTAs, four KPI cards (total/revenue/pending/delivered), filter bar (search/status/payment/date range/reset), dense orders table (order # + source, customer avatar, item thumbs, payment/order status badges, dates, row actions), pagination. Sidebar Orders expands to All Orders / Pending / Processing / Shipped / Delivered / Cancelled (`status` query filters). Fixtures only until Order admin Gateway
- **Sellers (`/sellers`):** Alloy Seller Management — breadcrumb, Add Seller CTA, five KPI cards (incl. commission), filter bar (search/status/verification/sort/reset), dense sellers table (shop, email, status/verification badges, orders/sales, row actions), pagination. Sidebar Sellers expands to All Sellers / Pending Approval / Suspended (`status` query filters). Fixtures only — Seller Gateway deferred
- **Seller Approvals (`/sellers/approvals`):** list of pending applications. Clicking a seller opens `/sellers/approvals/[id]` full review page with business/shop/verification/terms (mirrors web onboarding), admin checklist (Approve disabled until all checks pass), review notes, Reject / Approve Seller
- Charts are lightweight SVG presentation fixtures (no ECharts until a reporting API exists)
- Denser spacing, operational cards, shared tokens; active nav uses indigo→violet gradient pill
